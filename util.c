...
uint8_t util_add_new_measurent(util_quantized_low_pass_t *quantizized_low_pass, uint16_t measurement) {
    ...
    uint8_t raw_step = (uint8_t) (quantizized_low_pass->_mem / step_size);
    uint16_t delta_to_middle = abs(raw_step * step_size + step_size/2 - quantizized_low_pass->_mem);
...
uint16_t util_battery_voltage_milli_volt(void) {
    ...
    return last_voltage = (voltage + last_voltage * 3) / 4;
}
